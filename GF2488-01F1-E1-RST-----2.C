...
void ConfSet(void)
{
	...
	for(i=0;i<(LINENUM-1);i++){
		...
		if(g_ucConfData[64+i]==0){  // 支路关闭
			...
			XBYTE[sw1021Chip + PORT_CFG_REG(n)] = A_UP_DOWN | NRZSEL;
		...
		for(j=0;j<16;j++)
		{
			...
			// A 
			writeTxJ2(m, j, i,  temp|g_ucConfData[515+2*i*16+j], 0);
			writeExpectJ2(m, j, i,  temp|g_ucConfData[515+16+2*i*16+j], 0);
			// B
			writeTxJ2(m, j, i,  temp|g_ucConfData[515+2*i*16+j], 1);
			writeExpectJ2(m, j, i,  temp|g_ucConfData[515+16+2*i*16+j], 1);
		}
...
void InitioSw1021()
{
  ...
  first_conf=0;
	...
	for(i=0; i<CHIPS_ON_BOARD; i++)
	{
		...
		// 上下话数据总线均选择偶校验
		//XBYTE[SW1021 + SW1021_CHIP_ADDR(i) + SDH_CTRL_REG(A_BUS_BASE)] = 0x00;
		//XBYTE[SW1021 + SW1021_CHIP_ADDR(i) + SDH_CTRL_REG(B_BUS_BASE)] = 0x00;
		
		// 21个端口从A总线上下话, 使用 +- HDB3模式
		for(j=0; j<21; j++){
			XBYTE[SW1021_CHIP_ADDR(i) + PORT_CFG_REG(j)] = A_UP_DOWN;
			// 关闭线路环回和系统环回
		...
		// TODO:上下话时钟数据同相
		//XBYTE[SW1021_CHIP_ADDR(i) + E1_CTRL_REG] = ??
	...
	for(i=0;i<63;i++)            
	{
		...
		for(j=0;j<16;j++)
		{
			if(j==0)
			{ // A 
				writeTxJ2(m, j, i,  0x80, 0);
				writeExpectJ2(m, j, i,  0x80, 0);
				// B
				writeTxJ2(m, j, i,  0x80, 1);
				writeExpectJ2(m, j, i,  0x80, 1);           
			}
			else
			{
				// A 
				writeTxJ2(m, j, i,  0x00, 0);
				writeExpectJ2(m, j, i,  0x00, 0);
				// B
				writeTxJ2(m, j, i,  0x00, 1);
				writeExpectJ2(m, j, i,  0x00, 1); 
			}
		}
	}
...
static int checkSW1021Init(void)
{
	...
	for(i=0;i<CHIPS_ON_BOARD;i++)
	{
		...
		for(j=0; j<21; j++){
			reg = XBYTE[SW1021_CHIP_ADDR(i) + PORT_CFG_REG(j)] = A_UP_DOWN | NRZSEL;
			if((reg & (A_UP_DOWN | NRZSEL)) != (A_UP_DOWN | NRZSEL)){
				ret = -1;
				goto quit;
			}
		}
